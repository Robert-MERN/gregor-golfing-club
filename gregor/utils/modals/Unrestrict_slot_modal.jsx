...
const Unrestrict_slot_modal = ({ open, close }) => {
    const { booking_delete_id, cookieUser, handle_delete_user_booking_admin } = useStateContext()
    const handleDelete = () => {
        handle_delete_user_booking_admin(booking_delete_id, cookieUser.id, true);
        close();
    }

    return (
        <Dialog
            open={open.unrestrict_slot_modal}
            onClose={close}
        >
            <div className=' md:w-[500px] p-6 md:p-7 relative flex flex-col gap-8 md:gap-10' >
                ...
                <p className='text-[13px] md:text-[16px] text-stone-600 font-medium' >Are you sure, you want to Unrestrict slot?</p>
...
export default Unrestrict_slot_modal